...
def display_frames(vid_path, model, classes,image_name_exit="captured_image_exit.jpg",vid_out= None):
    cap = cv2.VideoCapture(vid_path)
    cv2.namedWindow("Exit Cam", cv2.WINDOW_NORMAL)
    frame_no = 1
    skip_frame = 1  # Skip every 5 frames
    resize_width = 640
    resize_height = 640
    while cap.isOpened():
        ret, frame = cap.read()
...
def check_extracted_text_in_database(extracted_text):
    extracted_text_str_exit = ' '.join(extracted_text) if isinstance(extracted_text, list) else extracted_text
    response = requests.get('http://127.0.0.1:5000/check_plate_exit', params={'extracted_text_str_exit': extracted_text_str_exit})
    print(response.text)
...
def save_image(frame, image_name_exit):
    if frame is None:
        print("Error: No frame to save")
        return
    # Save the captured frame as an image
    cv2.imwrite(image_name_exit, frame)
...
def main(img_path=None, vid_path=None,vid_out = None):
    ...
    #model =  torch.hub.load('./yolov5-master', 'custom', source ='local', path='PND.pt',force_reload=True) ### The repo is stored locally
    model = torch.hub.load('./yolov5', 'custom', source='local', path='sticker.pt',force_reload=True)  ### The repo is stored locally
...
#main(vid_path='rtsp://admin:admin@192.168.1.108:554/cam/realmonitor?channel=1&subtype=0', vid_out="output.avi") #### for webcam
