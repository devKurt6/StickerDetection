...
def display_frames(vid_path, model, classes,image_name_entry="captured_image_entry.jpg",vid_out= None):
    ...
    while cap.isOpened():
        ...
        if frame_no % skip_frame == 0:
            ...
            save_image(frame, image_name_entry)
        else:
        ...
        cv2.imshow("Entry Camera", frame_with_boxes)
...
def save_image(frame, image_name_entry):
    if frame is None:
        print("Error: No frame to save")
        return
    # Save the captured frame as an image
    cv2.imwrite(image_name_entry, frame)
...
def main(img_path=None, vid_path=None,vid_out = None):
    ...
    # model =  torch.hub.load('ultralytics/yolov5', 'custom', path='eggplant.pt',force_reload=True) ## if you want to download the git repo and then run the detection
...
#main(vid_path="./car.mp4",vid_out="vid_1.mp4") ### for custom video
